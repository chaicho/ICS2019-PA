...
static struct rule {
  ...
  {"\\|\\|",TK_OR},
  {"!=",TK_NEQ},
  {"\\$[Ee]?[A-Da-d][xX]|\\$[A-Da-d][xXlL]|\\$[Ee]?[sSdDBb][iIPp]",TK_REG},
};
...
static Token tokens[200] __attribute__((used)) = {}; //存放正则表达式
static int nr_token __attribute__((used))  = 0; //正则表达式的的数量
 bool address=false;
 int lowest=-1;// 用来记录当前优先级最低的符号
static int priority[32]={0,7,4,4,3,3,1,1,-1,-1,2,11,12,7,-1,2};
...
word_t expr(char *e, bool *success) {
  memset(tokens,0,200*sizeof(Token));
  if (!make_token(e)) {
    *success = false;
    return 0;
  }
